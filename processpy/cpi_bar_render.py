...
# 使用 Seaborn 的不同调色板
colors = sns.color_palette("coolwarm", n_colors=len(cities))  # 使用 coolwarm 调色板
...
if unit_price_data is None or unit_price_data.empty:
    ...
    # 设置标题和标签
    plt.xlabel("性价比 (CPI)", fontsize=14)
    plt.ylabel("城市", fontsize=14)
    plt.title("五个城市租房性价比 (CPI)", fontsize=16, fontweight="bold", pad=20)
    ...
    # 遍历每个柱子并添加标注
    for bar, row in zip(bars, mean_rent_sorted.itertuples()):  # 移除 strict=False
        # 获取柱子的中心x位置和顶部y位置
        x = bar.get_width()
        ...
        # 添加标注
        ax.text(
            x + 0.02 * mean_rent_sorted["性价比 (CPI)"].max(),  # 根据最大CPI值动态调整x位置
            y,
            annotation,
            color="black",
            ha="left",
    ...
    # 保存图表
    cpi_chart_filename = os.path.join(output_dir, "cpi_horizontal_bar_chart.png")
